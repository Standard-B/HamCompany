# 👷‍♀️求人&求職最大の人材市場 HamCompany👷‍♂️
### 人々が自身の才能と能力を幅広い仕事の世界で活用し、新たな挑戦を支援するシステム <br>
![메인페이지(로그인x)](https://github.com/Maengmo/HamCompany/assets/117720344/ada023b4-3757-4775-a9f0-585440977b6a)


<br>

## ⁉ プログラム紹介
  <p>人々が自分の才能や能力を広範な仕事の世界で活用し、新たな挑戦をサポートするプラットフォームを作ることに焦点を当てました。</p>
  <p>さまざまな業界や専門性を持つ人材が自分のプロフィールを登録し、それを通じて直接仕事を探し求める新しい形態の人材取引を探求しました。</p>
  <p>このアプローチは人材取引の透明性と効率性を向上させ、購入者にはワンクリックで人材を購入できる利便性を提供します。</p>
  <p>人々が自分の能力を最大限に活用し、より多くの機会を得ることを支援することが究極の目標です。</p>
  
  <br>
  
## ✔ 企画意図および主要業務
- ログイン, 会員登録
  - ログインの有効性検証、会員登録の有効性検証、Securityによって該当ユーザーに権限を付与します。<br><br>

- 人材を探す
  - 求職申し込みが完了した会員は求職市場に登録されます。事業者は選択した求職者から希望の求職者を選んで選択と支払いを行い、採用できます。支払いはカカオペイで行われます。<br><br>

- レビューボード
  - 求職経験者がレビューを投稿できるCRUD掲示板。<br><br>

- フリーボード
  - 事業者と求職者に分類されたCRUD掲示板で、自由に対話できます。<br><br>

- ポートフォリオ
  - 自身が進行したプロジェクトや仕事に関する情報を記録できる掲示板で、事業者はこれを通じてユーザーのポートフォリオを閲覧できます。<br><br>

- 求職者 マイページ
  - プロフィール、問い合わせ履歴、ポートフォリオ、応募履歴、レビューボード、フリーボードなど、CRUDが可能なマイページです。<br><br>
 
- 事業者 マイページ
  - 事業者プロフィール、事業者詳細プロフィール、ウィッシュリスト、レビュー、お問い合わせ履歴、注文履歴、フリーボードなど、CRUDが可能なマイページ

<br>

## 📆 開発期間
- 2023.06.20 - 2023.07.10

<br>

## 🖥 開発環境
### ✔ 使用言語
- JAVA (JDK 11), SQL, PLSQL, HTML, CSS, JavaScript
### ✔ 使用技術
- JDBC, AWS, Ajax, Spring, Boot Strap
### ✔ IDE
- Eclipse, SQLDeveloper, AWS, vscode, eXERD, Figma, STS
### ✔ 開発用 PC OS
- 2S version(platform) : Windows , Mac

<br>

## 📁 ERD 
![함해볼텨erd](https://github.com/Maengmo/HamCompany/assets/117720344/0c70aa87-a294-40f0-a4a2-7200af7f020a)


<br>

##  🖥 実装画面
<br>

#### 1️⃣ 로그인
<img width="954" alt="로그인" src="https://github.com/Maengmo/HamCompany/assets/117720344/56a078fb-2632-490e-b797-98e675afb47f">

<br><br>


#### 2️⃣ 회원가입
![회원가입](https://github.com/Maengmo/HamCompany/assets/117720344/114a7121-71ec-43d7-8404-1b63b5ca34e3)

<br>

#### 3️⃣ 인재 둘러보기
![쇼핑게시판 - 인재 둘러보기 리스트](https://github.com/Maengmo/HamCompany/assets/117720344/bdc54199-19ac-4fee-ace2-146d27bcd2f0)

<br>

#### 4️⃣ 결제
<img width="946" alt="결제 - 카카오페이 결제" src="https://github.com/Maengmo/HamCompany/assets/117720344/0ea82c30-7b8a-493a-bd3b-16f2bee7d8e0">

<br><br>

#### 5️⃣ 들어볼텨
![함들어볼텨 게시판](https://github.com/Maengmo/HamCompany/assets/117720344/85f5feaa-5dc8-4a6d-a97a-21aba0b4be83)

<br>

#### 6️⃣ 말해볼텨
![함말해볼텨(사업자) 게시판](https://github.com/Maengmo/HamCompany/assets/117720344/efce8325-2751-43eb-9c8a-19f618ea52d4)

<br>

#### 7️⃣ 포트폴리오
![포트폴리오 게시판](https://github.com/Maengmo/HamCompany/assets/117720344/69000031-7095-4b2e-a4cd-1ba3421dcf08)

<br>

#### 8️⃣ 구직회원 마이페이지
![마이페이지-사용자](https://github.com/Maengmo/HamCompany/assets/117720344/a8c3df78-0843-4ccf-be44-ac50be675a0e)

<br>

#### 9️⃣ 사업자회원 마이페이지
![함말해볼텨(사업자) 게시판](https://github.com/Maengmo/HamCompany/assets/117720344/5f69a02b-1744-4f93-b1f9-ae080deda12e)

<br>

#### 🔟 관리자
![관리자페이지-매칭관리](https://github.com/Maengmo/HamCompany/assets/117720344/3242084c-4f39-4532-85d3-a93e339e82e8)


<br>



